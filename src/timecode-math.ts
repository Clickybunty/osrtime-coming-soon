export type TimecodeFormat = '24' | '23.976' | '30' | '29.97NDF' | '29.97DF';
...
export const RATES: Record<TimecodeFormat, Fraction> = {
  '24': new Fraction(24),
...
export function parseTimecode(tc: string): { h: number; m: number; s: number; f: number; isDF: boolean } {
  const match = tc.match(/^(\d{2}):(\d{2}):(\d{2})([:;])(\d{2})$/);
  if (!match) throw new Error(`Invalid timecode format: ${tc}`);
  return {
...
function pad(n: number | bigint): string {
 ...
 */
export function timecodeToFrames(tc: string, format: TimecodeFormat): bigint {
 ...
 */
export function framesToTimecode(frames: bigint, format: TimecodeFormat): string {
 ...
 */
export function realtimeToFrames(seconds: Fraction, format: TimecodeFormat): Fraction {
 ...
 */
export function realtimeToDiscreteFrames(seconds: Fraction, format: TimecodeFormat): bigint {
 ...
 */
export function framesToRealtime(frames: bigint, format: TimecodeFormat): Fraction {
